...
def go(args):
    ...
    logger.info(f"Precision: {precision:0.2f}, Recall: {recall:0.2f}, F1: {f1:0.2f}")
    ...
    for feature in cat_features:
        slice_performance(test, y_test, y_pred, feature, args.results_output_path)
...
def slice_performance(df, y_true, y_pred, feature, path):
    ...
    plt.text(0.0, title_pos, f"Value",     {'fontsize': 10}, fontproperties = 'monospace', weight='bold')
    plt.text(0.5, title_pos, f"Precision", {'fontsize': 10}, fontproperties = 'monospace', weight='bold')
    plt.text(1.0, title_pos, f"Recall",    {'fontsize': 10}, fontproperties = 'monospace', weight='bold')
    plt.text(1.5, title_pos, f"F1",        {'fontsize': 10}, fontproperties = 'monospace', weight='bold')
    ...
    for slice_val in df[feature].unique():
    	...
    	# Plotting text
        plt.text(0.0, x_pos, f"{slice_val}",      {'fontsize': 10}, fontproperties = 'monospace')
        plt.text(0.5, x_pos, f"{precision:0.2f}", {'fontsize': 10}, fontproperties = 'monospace')
        plt.text(1.0, x_pos, f"{recall:0.2f}",    {'fontsize': 10}, fontproperties = 'monospace')
        plt.text(1.5, x_pos, f"{f1:0.2f}",        {'fontsize': 10}, fontproperties = 'monospace')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Perform data process and training")
